...
try {
    ...
    if ($existingUser) {
        ...
        // Create new user with complete fields (same as email/password registration)
        $insertStmt = $pdo->prepare("
            INSERT INTO users (
                firebase_uid, email, full_name, first_name, last_name, profile_picture_url, 
                login_method, auth_provider, role, is_verified, is_available, 
                verification_status, badge_status, id_verified, badge_acquired,
                created_at, updated_at
            ) 
            VALUES (?, ?, ?, ?, ?, ?, 'google', 'google', 'user', 1, 1, 'unverified', 'none', 0, 0, NOW(), NOW())
        ");
        $insertStmt->execute([$firebaseUid, $email, $fullName, $firstName, $lastName, $profilePicture]);
    ...
    echo json_encode([
        ...
        'user' => [
            ...
            'badge_status' => $user['badge_status'] ?? 'none',
